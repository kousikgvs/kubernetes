...
## Use Helm After the Cluster Exists

Helm works only after the cluster is running and `kubectl` can connect to it.

Add the chart repositories used in this guide:
...
Check available charts and installed releases:

```powershell
helm search repo cert-manager
helm search repo kserve
helm list -A
```
...
- Use `kubectl` to view nodes, pods, logs, and resource details.
- Use Helm to install and upgrade platform components such as cert-manager and KServe.
